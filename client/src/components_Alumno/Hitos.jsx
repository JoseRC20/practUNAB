...
export default function Hitos() {
  ...
  const fetchProfile = async (tk) => {
    ...
    if (cached && (cached._id || cached.id || (cached.user && (cached.user._id || cached.user.id)))) {
  ...
  const fetchHitos = async (tk, studentId) => {
    const res = await fetch(`${API_BASE}/hitos/student/${encodeURIComponent(String(studentId))}`, {
      headers: { Authorization: `Bearer ${tk}` }
    });
    if (res.status === 401) throw new Error('AUTH_401');
  ...
  const fetchProfileAndHitos = async () => {
    ...
    try {
      ...
      const studentProfileId =
        ...
        profile?.user?._id || profile?.user?.id || null;
  ...
  useEffect(() => { fetchProfileAndHitos(); /* eslint-disable-next-line */ }, []);
          ...
          <div className="card mb-3" key={n}>
            <div className="card-body">
              ...
              <input
                ...
                id={`hito${n}`}
                accept=".doc,.docx,.pdf"
                onChange={(e) => setTimeout(() => (n === 1 ? setHito1File : setHito2File)(e.target.files?.[0] || null), 0)}
                disabled={disabled}
              />

              {files.length > 0 && (
                <div className="text-muted small mb-2">
                  Archivo enviado: {files[files.length-1]?.originalName || files[files.length-1]?.originalname || files[files.length-1]?.name || 'Archivo'}
                </div>
              )}
