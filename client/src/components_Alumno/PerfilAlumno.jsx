...
const mapProfileToView = (p) => {
  const lastPractice =
    Array.isArray(p?.practices) && p.practices.length
...
export default function PerfilAlumno({ initialProfile }) {
  ...
  const fetchProfile = async () => {
    ...
    try {
      ...
      if (role && role !== "student") {
        alert(
          "Acceso denegado. Solo los alumnos pueden acceder a esta página."
        );
  ...
  const handleSupervisorEdit = () => {
    ...
    if (practiceStatus === "no iniciado" || practiceStatus === "") {
      alert(
        "No puede editar datos de práctica hasta que la práctica comience."
      );
      return;
    }
    setSnapshot({ ...view });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const nextProfile = {
        ...(profile || {}),
        practices:
          Array.isArray(profile?.practices) && profile.practices.length
            ? [
  ...
  const isApprovedBySecretary = (practiceStatus || "")
    .toLowerCase()
    .includes("aprob");
  ...
  return (
    <div className="container mt-5 mb-5">
      ...
      <span
        className="d-block w-50 bg-danger my-2 mx-auto mb-5"
        style={{ height: "5px" }}
      ></span>
      ...
      <div className="card">
        ...
        <div className="card-body">
          <h4 className="card-title">
            {`${view.name} ${view.lastNamePaternal || ""} ${
              view.lastNameMaternal || ""
            }`.trim()}
          </h4>
          ...
          <p className="card-text">
            <strong>Estado de la Práctica:</strong>{" "}
              ...
              {!isEditingSupervisor ? (
                <>
                  <p className="card-text">
                    ...
                    {view.supervisorName || "No asignado"}
                  </p>
                  <p className="card-text">
                    <strong>Correo del Supervisor:</strong>{" "}
                    {view.supervisorEmail || "No asignado"}
                  </p>
                  <p className="card-text">
                ...
                <>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setView({ ...view, supervisorName: e.target.value })
                    }
                  />
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setView({ ...view, supervisorEmail: e.target.value })
                    }
                  />
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setView({ ...view, endDate: e.target.value })
                    }
                    disabled={!isApprovedBySecretary}
                  />
                  {!isApprovedBySecretary && (
                    <div className="small text-muted">
                      La fecha de finalización solo puede ser editada si es
